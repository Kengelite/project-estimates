...
export default function DeleteCurriculumModal({ item, onClose, onConfirm }: DeleteCurriculumModalProps) {
  return (
    <ModalBase onClose={onClose}>
      ...
      <p className="text-sm text-gray-500 mb-4">คุณต้องการลบข้อมูลบริหารหลักสูตรนี้ใช่หรือไม่?</p>

      <div className="bg-gray-50 rounded-xl px-4 py-3 mb-3 text-sm space-y-1.5">
        <p><span className="text-gray-400">ชื่อบริหาร : </span><span className="text-gray-700">{item.name}</span></p>
        <div className="flex gap-3 text-xs pt-1">
          <p><span className="text-gray-400">ป.ตรี(ปกติ): </span><span className="text-gray-700">{item.bachelorNormal}</span></p>
          <p><span className="text-gray-400">ป.ตรี(พิเศษ): </span><span className="text-gray-700">{item.bachelorSpecial}</span></p>
          <p><span className="text-gray-400">บัณฑิต: </span><span className="text-gray-700">{item.master}</span></p>
        </div>
      </div>
