...
export interface StudentYearItem {
  id: number;
  studentYear: string;
  isActive: boolean;
}

interface DeleteStudentYearModalProps {
  item: StudentYearItem;
  onClose: () => void;
  onConfirm: () => void;
}
...
export default function DeleteStudentYearModal({
  item,
  onClose,
  onConfirm,
}: DeleteStudentYearModalProps) {
  return (
    <ModalBase onClose={onClose}>
      ...
      <p className="mb-4 text-sm text-gray-500">
        คุณต้องการลบข้อมูลชั้นปีนี้ใช่หรือไม่?
      </p>

      <div className="mb-3 space-y-1.5 rounded-xl bg-gray-50 px-4 py-3 text-sm">
        <p>
          <span className="text-gray-400">ชั้นปี : </span>
          <span className="text-gray-700">ปี {item.studentYear}</span>
        </p>
        <p>
