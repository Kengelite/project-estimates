...
export interface SubjectOutsideItem {
  id: string;
  subjectCode: string;
  subjectName: string;
  isActive: boolean;
...
interface DeleteSubjectOutsideModalProps {
  item: SubjectOutsideItem;
  onClose: () => void;
  onConfirm: () => void;
}
...
export default function DeleteSubjectOutsideModal({
  item,
  onClose,
  onConfirm,
}: DeleteSubjectOutsideModalProps) {
  return (
    <ModalBase onClose={onClose}>
      ...
      <p className="mb-4 text-sm text-gray-500">
        คุณต้องการลบข้อมูลรหัสวิชานอกคณะนี้ใช่หรือไม่?
      </p>

      <div className="mb-3 space-y-1.5 rounded-xl bg-gray-50 px-4 py-3 text-sm">
        <p>
          <span className="text-gray-400">รหัสวิชานอกคณะ : </span>
          <span className="text-gray-700">{item.subjectCode}</span>
        </p>
        <p>
          <span className="text-gray-400">ชื่อวิชานอกคณะ : </span>
