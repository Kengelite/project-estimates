...
export default function UniversityWorkPage() {
  ...
  const fetchUniversityWorks = async () => {
    try {
      ...
      await Swal.fire({
        ...
        text: error || "ไม่สามารถดึงข้อมูลงานมหาวิทยาลัยได้",
  ...
  const validateForm = () => {
    if (!formData.name.trim()) {
      return "กรุณากรอกชื่องานวิทยาลัย";
    }

    if (formData.name.trim().length > 150) {
      return "ชื่องานวิทยาลัยต้องไม่เกิน 150 ตัวอักษร";
    }
  ...
  const handleSubmit = async () => {
    ...
    const confirmResult = await Swal.fire({
      ...
      html: `
        ต้องการ${editingItem ? "แก้ไข" : "บันทึก"}งานวิทยาลัย <b>${nameValue}</b> ใช่หรือไม่<br/>
    ...
    try {
      ...
      setEditingItem(null);
      resetForm();
      await fetchUniversityWorks();

      await Swal.fire({
        icon: "success",
        title: "สำเร็จ",
        text: editingItem
          ? `แก้ไขข้อมูลงานวิทยาลัย "${nameValue}" เรียบร้อยแล้ว`
          : `เพิ่มข้อมูลงานวิทยาลัย "${nameValue}" เรียบร้อยแล้ว`,
        confirmButtonColor: "#22c55e",
      });
    } catch (error: any) {
      await Swal.fire({
  ...
  const handleDelete = async () => {
    ...
    const confirmResult = await Swal.fire({
      ...
      text: `ต้องการลบข้อมูลงานวิทยาลัย "${deletingItem.name}" ใช่หรือไม่`,
      showCancelButton: true,
    ...
    try {
      setSubmitting(true);
      await DeleteDataUniversityWork(deletingItem.id);
      ...
      await Swal.fire({
        icon: "success",
        title: "ลบสำเร็จ",
        text: "ลบข้อมูลงานวิทยาลัยเรียบร้อยแล้ว",
        confirmButtonColor: "#22c55e",
      });
    } catch (error: any) {
      await Swal.fire({
  ...
  const handleToggleStatus = async (item: WorkItem) => {
    ...
    const confirmResult = await Swal.fire({
      ...
      text: `ต้องการ${nextStatusText}งานวิทยาลัย "${item.name}" ใช่หรือไม่`,
      showCancelButton: true,
      confirmButtonText: "บันทึก",
      cancelButtonText: "ยกเลิก",
      confirmButtonColor: "#22c55e",
    ...
    try {
      await EditStatusUniversityWork(item.id, nextStatus);
      await fetchUniversityWorks();

      await Swal.fire({
        icon: "success",
        title: "สำเร็จ",
        text: `เปลี่ยนสถานะงานวิทยาลัย "${item.name}" เรียบร้อยแล้ว`,
        confirmButtonColor: "#22c55e",
      });
    } catch (error: any) {
      await Swal.fire({
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 sm:p-6">
      <div className="mx-auto space-y-5">
        ...
        {showFormModal && (
          <WorkFormModal
            title={editingItem ? "แก้ไขข้อมูลงานวิทยาลัย" : "เพิ่มงานวิทยาลัย"}
            iconColor="#3b82f6"
            formData={formData}
