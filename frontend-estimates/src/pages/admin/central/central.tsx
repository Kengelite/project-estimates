...
import CentralDeptFormModal, {
  type DeptFormData,
} from "./components/CentralDeptFormModal";
import DeleteDeptModal, { type Dept } from "./components/DeleteDeptModal";
import {
  GetDataCentral,
  AddDataCentral,
  EditDataCentral,
...
export default function CentralManagement() {
  ...
  const [editingItem, setEditingItem] = useState<Dept | null>(null);
  ...
  const fetchCentrals = async () => {
    try {
      ...
      await Swal.fire({
        ...
        text: error || "ไม่สามารถดึงข้อมูลส่วนกลางได้",
  ...
  const validateForm = () => {
    if (!formData.name.trim()) {
      return "กรุณากรอกชื่อส่วนกลางวิทยาลัย";
    }

    if (formData.name.trim().length > 150) {
      return "ชื่อส่วนกลางต้องไม่เกิน 150 ตัวอักษร";
    }
  ...
  const handleOpenEdit = (item: Dept) => {
    setEditingItem(item);
    setFormData({
      name: item.name,
  ...
  const handleSubmit = async () => {
    ...
    const confirmResult = await Swal.fire({
      ...
      html: `
        ต้องการ${editingItem ? "แก้ไข" : "บันทึก"}ส่วนกลาง <b>${nameValue}</b> ใช่หรือไม่<br/>
    ...
    try {
      ...
      await Swal.fire({
        ...
        title: "สำเร็จ",
        text: editingItem
          ? `แก้ไขส่วนกลาง "${nameValue}" เรียบร้อยแล้ว`
          : `เพิ่มส่วนกลาง "${nameValue}" เรียบร้อยแล้ว`,
        confirmButtonColor: "#22c55e",
      });
    } catch (error: any) {
      await Swal.fire({
  ...
  const handleDelete = async () => {
    ...
    const confirmResult = await Swal.fire({
      ...
      text: `ต้องการลบข้อมูลส่วนกลาง "${deletingItem.name}" ใช่หรือไม่`,
      showCancelButton: true,
    ...
    try {
      setSubmitting(true);
      await DeleteDataCentral(deletingItem.id);
      ...
      await Swal.fire({
        icon: "success",
        title: "ลบสำเร็จ",
        text: "ลบข้อมูลส่วนกลางเรียบร้อยแล้ว",
        confirmButtonColor: "#22c55e",
      });
    } catch (error: any) {
      await Swal.fire({
  ...
  const handleToggleStatus = async (item: Dept) => {
    ...
    const confirmResult = await Swal.fire({
      ...
      text: `ต้องการ${nextStatusText}ส่วนกลาง "${item.name}" ใช่หรือไม่`,
      showCancelButton: true,
      confirmButtonText: "บันทึก",
      cancelButtonText: "ยกเลิก",
      confirmButtonColor: "#22c55e",
    ...
    try {
      await EditStatusCentral(item.id, nextStatus);
      await fetchCentrals();

      await Swal.fire({
        icon: "success",
        title: "สำเร็จ",
        text: `เปลี่ยนสถานะส่วนกลาง "${item.name}" เรียบร้อยแล้ว`,
        confirmButtonColor: "#22c55e",
      });
    } catch (error: any) {
      await Swal.fire({
      ...
      <div className="mx-auto space-y-5">
        <nav className="mb-4 text-sm text-gray-400">
          ...
          <span className="font-medium text-gray-700">
            จัดการบริหารส่วนกลางวิทยาลัย
          </span>
        </nav>

        <div className="mb-5 flex items-center justify-between">
          <h1 className="text-xl font-bold text-gray-900">
            บริหารส่วนกลางวิทยาลัย
          </h1>
        ...
        {showFormModal && (
          <CentralDeptFormModal
            title={editingItem ? "แก้ไขข้อมูลส่วนกลาง" : "เพิ่มส่วนกลาง"}
            iconColor="#3b82f6"
            formData={formData}
