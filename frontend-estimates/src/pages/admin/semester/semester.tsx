...
import DeleteSemesterModal, {
  type SemesterItem,
} from "./components/DeleteSemesterModal";
import {
  GetDataSemester,
  AddDataSemester,
  EditDataSemester,
...
function SortableHeader({
  ...
  return (
    <button
      ...
      className={`inline-flex items-center gap-1 text-xs font-semibold text-gray-600 transition-colors hover:text-blue-600 ${
...
export default function SemesterManagement() {
  ...
  const [formData, setFormData] = useState<SemesterFormData>(emptyForm);

  const mapSemesterToItem = (item: SemesterApiItem): SemesterItem => {
    return {
  ...
  const fetchSemesters = async () => {
    try {
      ...
      const response = await GetDataSemester();
      const list = pickArrayFromResponse<SemesterApiItem>(response);

      setItems(
        list
          .map((item) => mapSemesterToItem(item))
          .filter((item) => Boolean(item.id) && Boolean(item.semester)),
      );
    } catch (error: any) {
      await Swal.fire({
        icon: "error",
        title: "เกิดข้อผิดพลาด",
        text: error || "ไม่สามารถดึงข้อมูลภาคการศึกษาได้",
  ...
  const sortedItems = useMemo(() => {
    ...
    rows.sort((a, b) => {
      ...
      if (sortConfig.key === "semester") {
        result = compareText(a.semester, b.semester);
      }
  ...
  const validateForm = () => {
    if (!formData.semester.trim()) {
      return "กรุณากรอกชื่อภาคการศึกษา";
    }

    if (formData.semester.trim().length > 100) {
      return "ชื่อภาคการศึกษาต้องไม่เกิน 100 ตัวอักษร";
    }
  ...
  const handleOpenEdit = (item: SemesterItem) => {
    setEditingItem(item);
    setFormData({
  ...
  const handleSubmit = async () => {
    const value = formData.semester.trim();
    ...
    const confirmResult = await Swal.fire({
      ...
      text: editingItem
        ? `ต้องการแก้ไขภาคการศึกษา "${value}" ใช่หรือไม่`
        : `ต้องการบันทึกภาคการศึกษา "${value}" ใช่หรือไม่`,
    ...
    try {
      ...
      if (editingItem) {
        await EditDataSemester(editingItem.id, {
          semester: value,
          status: editingItem.isActive ? "1" : "0",
        });
      } else {
        await AddDataSemester({
          semester: value,
          status: "1",
        });
      }
      ...
      await Swal.fire({
        ...
        title: "สำเร็จ",
        text: editingItem
          ? `แก้ไขภาคการศึกษา "${value}" เรียบร้อยแล้ว`
          : `เพิ่มภาคการศึกษา "${value}" เรียบร้อยแล้ว`,
        confirmButtonColor: "#22c55e",
      });
  ...
  const handleDelete = async () => {
    ...
    const confirmResult = await Swal.fire({
      ...
      text: `ต้องการลบภาคการศึกษา "${deletingItem.semester}" ใช่หรือไม่`,
      showCancelButton: true,
    ...
    try {
      ...
      await DeleteDataSemester(deletingItem.id);
      ...
      await Swal.fire({
        icon: "success",
        title: "ลบสำเร็จ",
        text: "ลบข้อมูลภาคการศึกษาเรียบร้อยแล้ว",
        confirmButtonColor: "#22c55e",
      });
  ...
  const handleToggleStatus = async (item: SemesterItem) => {
    ...
    const confirmResult = await Swal.fire({
      ...
      text: `ต้องการ${nextStatusText}ภาคการศึกษา "${item.semester}" ใช่หรือไม่`,
      showCancelButton: true,
      confirmButtonText: "บันทึก",
      cancelButtonText: "ยกเลิก",
      confirmButtonColor: "#22c55e",
    ...
    try {
      await EditStatusSemester(item.id, nextStatus);
      ...
      await Swal.fire({
        icon: "success",
        title: "สำเร็จ",
        text: `เปลี่ยนสถานะภาคการศึกษา "${item.semester}" เรียบร้อยแล้ว`,
        confirmButtonColor: "#22c55e",
      });
    } catch (error: any) {
      await Swal.fire({
    ...
    <div className="min-h-screen bg-gray-50 p-4 sm:p-6">
      <div className="mx-auto space-y-5">
        <nav className="mb-4 text-sm text-gray-400">
          ...
          <span className="font-medium text-gray-700">จัดการภาคการศึกษา</span>
        </nav>
