...
type CentralSplit struct {
	...
	CentralID    uuid.UUID      `gorm:"type:char(36);not null;index:idx_central_split_group,unique" json:"centralId"`
	Central      Central        `gorm:"foreignKey:CentralID;references:ID" json:"central"`
	SplitGroupID uuid.UUID      `gorm:"type:char(36);not null;index:idx_central_split_group,unique" json:"splitGroupId"`
